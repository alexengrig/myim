...
public interface ChatMessage2RemovingChatMessageResponseConverter
        extends Converter<ChatMessage, RemovingChatMessageResponse> {

    @Mapping(target = "chatId", source = "chat.id")
    @Mapping(target = "authorId", source = "author.id")
    @Mapping(target = "authorName", source = "author.name")
    @Override
    RemovingChatMessageResponse convert(ChatMessage source);
